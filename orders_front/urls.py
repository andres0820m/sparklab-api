...
from orders.views import OrderViewSet, home, edit_order, save_order, delete_order, approve_order, running_fail, \
    ...
    GetBank, GetAccount, GetDocument, GetUser, GetAds, GetAmount, ads, edit_ad, save_ad, save_amount, GetNotification
...
urlpatterns = [
    ...
    path('status/', home),
    path('ads/', ads),
    path('notifications/', GetNotification.as_view()),
    ...
    path('edit_order/<binance_id>', edit_order),
    path('approve_order/<binance_id>', approve_order),
    path('delete_order/<binance_id>', delete_order),
